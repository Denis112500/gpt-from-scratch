...
class GPT(nn.Module):
    def __init__(self, vocab_size, d_model, num_heads, num_layers, max_seq_len):
        ...
        # lower-triangular causal mask — 1 = visible, 0 = masked out.
        # registered as a buffer so it moves with .to("cuda") but isn't a parameter.
        # shape (1, 1, T, T) broadcasts over (batch, num_heads, T, T) scores.
    ...
    def forward(self, x, mask=None):
        seq_len = x.shape[1]
        if mask is None:
            mask = self.causal_mask[:, :, :seq_len, :seq_len] # crop to this batch's length
        positions = torch.arange(seq_len, device=x.device) # [0, 1, 2, ..., seq_len]
